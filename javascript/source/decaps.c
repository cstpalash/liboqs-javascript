...
int Decaps(const char *algo){

	char secretKeyFile[] = SECRET_KEY_FILE_NAME;
	char aesKeyFile[] = AES_KEY_SERVER_FILE_NAME;
	...
	if(!readFile(secret_key, kem->length_secret_key, secretKeyFile)){
		fprintf(stderr, "ERROR: secret key file read failed!\n");
		cleanup_heap(secret_key, shared_secret_e, shared_secret_d, public_key,
		             ciphertext, kem);
	...
	if(!readFile(ciphertext, kem->length_ciphertext, cipherTextFile)){
		fprintf(stderr, "ERROR: Cipher text file read failed!\n");
		cleanup_heap(secret_key, shared_secret_e, shared_secret_d, public_key,
		             ciphertext, kem);
	...
	if(!writeFile(shared_secret_d, kem->length_shared_secret, aesKeyFile)){
